 ...
 * Uses the system's VagabondDamageHelper.calculateFinalDamage().
...
export async function useConsumable(actor, item) {
  ...
  try {
    if (effect.type === "removeStatus") {
      ...
      if (alcData?.level >= 4) {
        try {
          const { VagabondDamageHelper: DmgHelper } = globalThis.vagabond.utils ?? {};
          if (DmgHelper?._manuallyExplodeDice) {
            ...
            await DmgHelper._manuallyExplodeDice(roll, explodeValues);
          }
        } catch (e) {
          console.warn(`${SYSTEM_ID} | Exploding dice on heal failed:`, e);
        }
      }
    ...
    console.log(`${SYSTEM_ID} | Used consumable: ${item.name} on ${actor.name}`);
    return true;

  } catch (err) {
    console.error(`${SYSTEM_ID} | Failed to use consumable "${item.name}":`, err);
...
export async function applyOnHitEffects(effect, targetActor) {
  ...
  for (const fx of effect.onHitEffects) {
    ...
    try {
      ...
      console.log(`${SYSTEM_ID} | Applied "${fx.label}" to ${targetActor.name}`);
    } catch (err) {
      console.warn(`${SYSTEM_ID} | Failed to apply "${fx.label}" to ${targetActor.name}:`, err);
    }
  }
}
...
export async function applySplashDamage(effect, primaryTarget, primaryDamage, attackerActor) {
  ...
  if (!primaryTokenObj) {
    console.warn(`${SYSTEM_ID} | Splash: primary target token not found on canvas`);
    return;
  }
  ...
  if (splashDamage > 0) {
    for (const tok of splashTargets) {
      ...
      try {
        ...
        console.warn(`${SYSTEM_ID} | Splash damage failed for ${splashActor.name}:`, err);
      }
    }
  }
  ...
  if (splashResults.length > 0) {
    ...
    await ChatMessage.create({
      ...
      speaker: { alias: "Vagabond" },
    });
  }

  console.log(`${SYSTEM_ID} | Splash: ${splashResults.length} nearby targets hit for ${splashDamage} each`);
}
...
export async function fetchCompendiumItems() {
  ...
  if (!pack) {
    console.warn(`${SYSTEM_ID} | Compendium 'vagabond.alchemical-items' not found.`);
    return [];
  }
...
export function getAlchemistData(actor) {
  ...
  // Backwards compat: try vagabond flag first, fall back to vagabond-crawler
  const formulae = tools?.getFlag('vagabond', 'knownFormulae') ?? tools?.getFlag('vagabond-crawler', 'knownFormulae') ?? [];
...
export function registerMaterialsHook() {
  Hooks.on("createItem", async (item, _options, _userId) => {
    ...
    console.log(`${SYSTEM_ID} | Auto-converted Materials to consumable: ${silver}s`);
  });
}
...
export async function craftItem(actor, itemName, isFormula) {
  ...
  if (effect && createdItem) {
    await createdItem.setFlag('vagabond', "alchemicalEffect", effect);
  }
  ...
  if (createdItem && isOffensiveType(itemData) && alcData.level >= 4) {
    ...
    console.log(`${SYSTEM_ID} | Applied ${alcData.level >= 8 ? "Big Bang" : "Potency"} to ${createdItem.name}: explode=${updates["system.explodeValues"]}`);
  }
...
export function registerCountdownDamageHook() {
  Hooks.on("createChatMessage", async (message) => {
    ...
    console.log(`${SYSTEM_ID} | createChatMessage hook — checking for countdown dice. Has "Countdown Dice": ${content.includes("Countdown Dice")}, Has "countdown": ${content.toLowerCase().includes("countdown")}`);

    // Check if this is a countdown dice chat card
    if (!content.toLowerCase().includes("countdown")) return;

    console.log(`${SYSTEM_ID} | Countdown chat detected. Content snippet:`, content.substring(0, 500));
    ...
    console.log(`${SYSTEM_ID} | Extracted dice name:`, diceName);
    if (!diceName) return;
    ...
    const journal = game.journal.find(j => {
      ...
      // Check both vagabond and vagabond-crawler flag scopes for backwards compat
      return !!j.flags?.vagabond?.countdownDamage || !!j.flags?.['vagabond-crawler']?.countdownDamage;
    });

    console.log(`${SYSTEM_ID} | Journal match:`, journal ? `${journal.name} (id: ${journal.id})` : "NOT FOUND");
    if (!journal) {
      ...
      console.log(`${SYSTEM_ID} | All countdown dice:`, allCD.map(j => ({
        name: j.flags.vagabond.countdownDice.name,
        hasOurFlags: !!j.flags?.vagabond?.countdownDamage || !!j.flags?.['vagabond-crawler']?.countdownDamage,
      })));
      return;
    }

    // Read from vagabond flags first, fall back to vagabond-crawler
    const dmgData = journal.flags?.vagabond?.countdownDamage ?? journal.flags?.['vagabond-crawler']?.countdownDamage;
    ...
    console.log(`${SYSTEM_ID} | Roll result:`, rollResult, "Damage data:", dmgData);
    ...
    if (!targetActor) {
      console.warn(`${SYSTEM_ID} | Target actor not found: ${dmgData.targetActorId}`);
      return;
    }
    ...
    if (dmgData.damageType) {
      ...
      console.log(`${SYSTEM_ID} | Countdown damage applied: ${finalDmg} ${dmgData.damageType} to ${targetActor.name}`);
    }
  });
}
...
export function registerCountdownLinkedAEHook() {
  Hooks.on("deleteJournalEntry", async (journal) => {
    ...
    if (dmgData.oilWeaponId && dmgData.oilActorId) {
      try {
        ...
        if (oilActor) {
          ...
          if (weapon) {
            // Restore original token light — check both flag scopes
            const origLight = weapon.flags?.vagabond?.oilOriginalLight ?? weapon.flags?.['vagabond-crawler']?.oilOriginalLight;
            ...
            // Restore original damage type if silvered (Anointing Oil) — check both flag scopes
            const origDmgType = weapon.flags?.vagabond?.oilOriginalDamageType ?? weapon.flags?.['vagabond-crawler']?.oilOriginalDamageType;
            if (origDmgType !== undefined) {
              await weapon.update({ "system.damageType": origDmgType });
              await weapon.unsetFlag('vagabond', "oilOriginalDamageType");
              try { await weapon.unsetFlag('vagabond-crawler', "oilOriginalDamageType"); } catch (_) { /* ignore */ }
            }
            // Restore original metal if it was changed — check both flag scopes
            const origMetal = weapon.flags?.vagabond?.oilOriginalMetal ?? weapon.flags?.['vagabond-crawler']?.oilOriginalMetal;
            if (origMetal !== undefined) {
              await weapon.update({ "system.metal": origMetal });
              await weapon.unsetFlag('vagabond', "oilOriginalMetal");
              try { await weapon.unsetFlag('vagabond-crawler', "oilOriginalMetal"); } catch (_) { /* ignore */ }
            }
            await weapon.unsetFlag('vagabond', "oilOriginalLight");
            await weapon.unsetFlag('vagabond', "oilIgnited");
            await weapon.unsetFlag('vagabond', "oilCoating");
            // Clean up legacy flags too
            try { await weapon.unsetFlag('vagabond-crawler', "oilOriginalLight"); } catch (_) { /* ignore */ }
            try { await weapon.unsetFlag('vagabond-crawler', "oilIgnited"); } catch (_) { /* ignore */ }
            try { await weapon.unsetFlag('vagabond-crawler', "oilCoating"); } catch (_) { /* ignore */ }

            ChatMessage.create({
              content: `<p>The burning oil on <strong>${weapon.name}</strong> has burned out.</p>`,
              speaker: { alias: "Vagabond" },
            });
            console.log(`${SYSTEM_ID} | Oil burned out on ${weapon.name}`);
          }
        }
      } catch (err) {
        console.warn(`${SYSTEM_ID} | Failed to clean up oil weapon flags:`, err);
      }
    }
    ...
    if (ae) {
      try {
        await ae.delete();
        console.log(`${SYSTEM_ID} | Removed linked AE "${ae.name}" from ${targetActor.name} (countdown ended)`);
        ChatMessage.create({
          content: `<p><strong>${ae.name}</strong> has ended on <strong>${targetActor.name}</strong>.</p>`,
          speaker: { alias: "Vagabond" },
        });
      } catch (err) {
        console.warn(`${SYSTEM_ID} | Failed to remove linked AE:`, err);
      }
    }
  });
}
...
export function registerOilBonusDamageHook() {
  Hooks.on("createChatMessage", async (message) => {
    ...
    // Check for oil ignited flag — check both flag scopes for backwards compat
    const oilData = weapon.flags?.vagabond?.oilIgnited ?? weapon.flags?.['vagabond-crawler']?.oilIgnited;
    if (!oilData) return;

    console.log(`${SYSTEM_ID} | Oil bonus damage hook fired for ${weapon.name}`);
    ...
    try {
        ...
        if (targetActor) {
          ...
          if (oilData.burnsTarget && oilData.burnsTargetDie) {
            try {
              ...
              console.log(`${SYSTEM_ID} | Bladefire applied Burning (${oilData.burnsTargetDie}) to ${targetActor.name}`);
            } catch (cdErr) {
              console.warn(`${SYSTEM_ID} | Failed to create Bladefire burning die:`, cdErr);
            }
          }
        }
      }
      ...
      console.warn(`${SYSTEM_ID} | Oil bonus damage failed:`, err);
    }
  });
}
...
export async function applyAlchemicalPostAttack(weapon, attackerActor, targetInfo, damageTotal) {
  // Check both flag scopes for backwards compat with existing crafted items
  const flagEffect = weapon.flags?.vagabond?.alchemicalEffect ?? weapon.flags?.['vagabond-crawler']?.alchemicalEffect;
  ...
  if (effect.countdownDie) {
    ...
    if (effect.onlyTargetBeingTypes?.length || effect.onlyTargetNames?.length) {
      ...
      console.log(`${SYSTEM_ID} | Target filter: name="${targetName}", beingType="${targetActor?.system?.beingType ?? "?"}", matchesType=${matchesType}, matchesName=${matchesName}`);
    ...
    if (createCountdown) {
      try {
        ...
        if (cdJournal) {
          ...
          if (effect.linkedStatus && targetActor) {
            const aeData = {
              ...
              origin: `system.vagabond.countdown.${cdJournal.id}`,
              disabled: false,
            ...
            try {
              ...
              console.log(`${SYSTEM_ID} | Applied "${effect.linkedStatus.label}" to ${targetActor.name}`);
            } catch (aeErr) {
              console.warn(`${SYSTEM_ID} | Failed to apply linked status:`, aeErr);
            }
          }

          await cdJournal.setFlag('vagabond', "countdownDamage", flagData);
        ...
        console.warn(`${SYSTEM_ID} | Failed to create countdown die:`, cdErr);
      }
    }
  }
...
export function registerAlchemicalAttackHook() {
  Hooks.on("createChatMessage", async (message) => {
    ...
    // Only process weapons with alchemical effects — check both flag scopes
    const effect = weapon.flags?.vagabond?.alchemicalEffect ?? weapon.flags?.['vagabond-crawler']?.alchemicalEffect;
    if (!effect) return;
    ...
    console.log(`${SYSTEM_ID} | Alchemical attack hook: ${weapon.name} → ${targetInfo?.actor?.name ?? "no target"}`);
...
export function registerConsumableUseHook() {
  Hooks.on("createChatMessage", async (message) => {
    ...
    console.log(`${SYSTEM_ID} | Consumable use detected: ${itemName}`);
    ...
    try {
        ...
        if (alcData?.level >= 4) {
          try {
            ...
            if (DmgHelper?._manuallyExplodeDice) {
              ...
              await DmgHelper._manuallyExplodeDice(roll, explodeVals);
            }
          } catch (e) { /* non-fatal */ }
        }
      ...
      console.error(`${SYSTEM_ID} | Consumable effect failed:`, err);
    }
  });
}
...
export function registerEurekaHook() {
  Hooks.on("createChatMessage", async (message) => {
    ...
    console.log(`${SYSTEM_ID} | Eureka! ${actor.name} gained a Studied die (${current} → ${current + 1})`);
  });
}
 ...
 * Called once from vagabond.mjs ready hook.
 */
  ...
  Hooks.on("updateCombat", async (combat, changed) => {
    ...
    for (const combatant of combat.combatants) {
      ...
      for (const ae of actor.effects) {
        // Check both origin scopes for backwards compat
        if (ae.origin !== `system.vagabond` && ae.origin !== `module.vagabond-crawler`) continue;
      ...
      if (toDelete.length) {
        try {
          ...
          console.log(`${SYSTEM_ID} | Expired ${toDelete.length} effect(s) on ${actor.name}`);
        } catch (err) {
          console.warn(`${SYSTEM_ID} | Failed to expire effects on ${actor.name}:`, err);
        }
      }
    }
  });
  console.log(`${SYSTEM_ID} | Effect expiration hook registered.`);
}
 ...
 *     "./systems/vagabond/module/helpers/alchemy-helpers.mjs"
 *   );
...
export async function populateAlchemicalFolder() {
  ...
  const FOLDER_NAME = "Alchemical Weapons (Vagabond)";
  ...
  if (!folder) {
    ...
    console.log(`${SYSTEM_ID} | Created folder: ${FOLDER_NAME}`);
  }
  ...
  if (toCreate.length) {
    ...
    for (const item of created_items) {
      ...
      if (effect) {
        await item.setFlag('vagabond', "alchemicalEffect", effect);
      }
    }
  }

  ui.notifications.info(`Alchemical Items: ${created} created, ${skipped} already existed.`);
  console.log(`${SYSTEM_ID} | Populated folder "${FOLDER_NAME}": ${created} created, ${skipped} skipped.`);
}
