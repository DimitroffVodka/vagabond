...
export async function performVirtuoso(actor, targetsAtRollTime = []) {
  ...
  if (isSuccess) {
    ...
    for (const pc of scenePCs) {
      ...
      // Toggle visible status icon on token
      const statusId = `virtuoso-${benefit}`;
      if (!pc.statuses?.has(statusId)) {
        await pc.toggleStatusEffect(statusId, { active: true });
      }
...
export async function expireVirtuosoBuffs() {
  ...
  for (const pc of allPCs) {
    ...
    if (hasVirtuosoBuff) {
      // Remove visible status icon
      const activeBenefit = pc.getFlag('vagabond', 'virtuosoBenefit');
      if (activeBenefit) {
        const statusId = `virtuoso-${activeBenefit}`;
        if (pc.statuses?.has(statusId)) {
          await pc.toggleStatusEffect(statusId, { active: false });
        }
      }
