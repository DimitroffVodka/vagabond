...
export class VagabondChatCard {
    ...
    actionIndex = null
      ...
      if (damageRoll) {
          ...
          let btns = isHealing
            ...
            : VagabondDamageHelper.createSaveButtons(damageRoll.total, damageType, damageRoll, actor.id, item?.id, attackType, targetsAtRollTime, actionIndex, rollData?.isCritical ?? false, statusSaveTypes, isCleave, damageRoll.sneakAttackDice || 0);
          ...
          card.addFooterAction(btns);
           ...
           const btn = VagabondDamageHelper.createDamageButton(actor.id, item.id, formula, {
               ...
               attackType,
               statKey,
               favorHinder: rollData.favorHinder || 'none'
           }, targetsAtRollTime);

           // ── Brawl Intent Handling ──────────────────────────────────────
           if (brawlIntent === 'grapple') {
               // Grapple: apply Restrained to targets (skip damage button)
               const appliedNames = [];
               const immuneNames = [];
               ...
               for (const t of targetsAtRollTime) {
                   ...
                   if (targetActor) {
                       ...
                       if (immunities.includes('restrained')) {
                           ...
                           // Create Restrained AE with grappler tracking
                           ...
                           // Bully perk: auto-create "Grappled Creature" weapon
                           if (hasBully) {
  ...
  static async weaponAttack(actor, weapon, attackResult, damageRoll, targetsAtRollTime = []) {
      ...
      const brawlIntent = attackResult.brawlIntent || 'damage';
