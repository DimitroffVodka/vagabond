...
export class VagabondDamageHelper {
  ...
  static calculateFinalDamage(actor, damage, damageType, attackingWeapon = null, sneakDice = 0, incomingDiceCount = 0, breakdown = null) {
    ...
    if (normalizedType === '-') {
      ...
      if (breakdown) breakdown.armorReduction = Math.min(armorRating, finalDamage); // actual armor absorbed
  ...
  static async handleSaveRoll(button, event = null) {
    ...
    for (const targetActor of actorsToRoll) {
      ...
      const finalDamage = this.calculateFinalDamage(targetActor, damageAfterSave, damageType, sourceItem, sneakDice, incomingDiceCount, dmgBreakdown);
   ...
   * @private
   */
  static async _postSaveResult(actor, saveType, roll, difficulty, isSuccess, isCritical, isHindered, originalDamage, saveReduction, armorReduction, finalDamage, damageType, autoApplied, rageReduction = 0) {
  ...
  static async handleApplyDirect(button) {
    ...
    if (isCleave && targetedTokens.length >= 2) {
      ...
      for (const { target, share } of shares) {
        ...
        const finalDamage = this.calculateFinalDamage(targetActor, share, damageType, sourceItem, sneakDice, diceCount);
        const currentHP = targetActor.system.health?.value || 0;
        const newHP = Math.max(0, currentHP - finalDamage);
        await targetActor.update({ 'system.health.value': newHP });
      ...
      for (const target of targetedTokens) {
        ...
        const finalDamage = this.calculateFinalDamage(targetActor, damageAmount, damageType, sourceItem, sneakDice, diceCount);
        const currentHP = targetActor.system.health?.value || 0;
        const newHP = Math.max(0, currentHP - finalDamage);
        await targetActor.update({ 'system.health.value': newHP });
  ...
  // ═══════════════════════════════════════════════════════════════════════
  //  ON-HIT EFFECTS: BURNING & STATUS (shared countdown die)
  // ═══════════════════════════════════════════════════════════════════════

  /**
   * Apply on-hit effects (burning, status, or both) to a target actor.
   * Burning and Status weapon properties share the same countdown die.
   *
   * Sources checked in order:
   * 1. Explicit overrides (damageType + dieTypeOverride params)
   * 2. Weapon properties + flags (Burning/Status in system.properties + flags.vagabond.onHitBurning)
   * 3. Actor-level relic power (system.onHitBurningDice, always fire)
   *
   * Stacking rules (burning only):
   * - Same damage type: keep the higher die (upgrade if new is bigger)
   * - Different damage types: both stack (separate burning dice)
   *
   * @param {Actor} targetActor - The actor that was hit
   * @param {Actor} sourceActor - The actor that dealt the hit
   * @param {string} [damageType] - The damage type for burning
   * @param {string} [dieTypeOverride] - Override die type
   * @param {Item} [sourceItem] - The weapon/spell item used
   */
