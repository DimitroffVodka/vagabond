...
// ── Effect Definitions ─────────────────────────────────────────────────────
// Each entry becomes a top-level ActiveEffect document in V14's primary-AE-doc
// compendium. The wrapper shape below is human-readable; populate() lifts
// effects[0] up, converts numeric modes to V14 type strings, and writes to
// system.changes per the V14 schema.
...
/**
 * Deterministic 16-char alphanumeric ID derived from an effect's name.
 * Foundry IDs are 16 chars. Stable across populate runs so module-side
 * UUID references (e.g. `Compendium.vagabond.effects.ActiveEffect.<id>`)
 * survive re-seeding.
 *
 * Format: 10-char readable slug + 6-char deterministic hash. The hash
 * disambiguates names that share their first 10 alphanumeric chars
 * (e.g. "Might +1" / "Might -1", "Movement - Swiftness I" / "II" / "III")
 * since truncation alone collides those.
 */
function _stableIdFromName(name) {
  const slug = name.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 10).padEnd(10, '0');
  let h = 5381;
...
/**
 * Effects whose `category` is in this set get `showIcon: 'never'` —
 * these are pure-mechanics buffs/debuffs that shouldn't clutter the token
 * row. Statuses (conditions) always get the icon.
 */
const HIDE_ICON_CATEGORIES = new Set(['buff', 'debuff', 'classFeature', 'material']);
...
export class EffectsCompendium {
  ...
  /**
   * Transform a single EFFECT_DEFINITIONS entry into V14-native AE document data.
   * Lifts effects[0] up, converts numeric modes → type strings, moves changes
   * to system.changes, hand-picks a deterministic _id, sets canonical flags.
   * Multi-effect entries fall back to the first effect (warns).
   */
  static _toV14EffectData(def, folderId) {
    const inner = (def.effects ?? [])[0];
    if (!inner) return null;
    if ((def.effects ?? []).length > 1) {
      console.warn(`EffectsCompendium | "${def.name}" has multiple inner effects; only effects[0] is migrated.`);
    }

    const cat = def.category || 'misc';
    const _id = _stableIdFromName(def.name);

    const changes = (inner.changes ?? []).map(c => {
      const type = (typeof c.mode === 'number') ? (MODE_TO_TYPE[c.mode] ?? 'add')
                 : (c.type ?? c.mode ?? 'add');
      const out = { key: c.key, type, value: String(c.value ?? '') };
      if (c.priority != null) out.priority = c.priority;
      return out;
    });

    // Human-readable canonical slug (no hash) for module lookup by short name.
    // Modules can find effects by `flags.vagabond.canonicalId` even if a GM
    // duplicates the compendium for editing (the duplicate gets a new _id but
    // keeps the canonicalId flag).
    // Preserve +/- signs as words so "Might +1" and "Might -1" don't both
    // collapse to "might-1". Strips other punctuation, collapses whitespace.
    const canonicalSlug = def.name
      .toLowerCase()
      .replace(/\+/g, ' plus ')
    ...
    return {
      _id,
      name: inner.name || def.name,
      img: inner.img || def.img || 'icons/svg/aura.svg',
      description: def.description ? `<p>${def.description}</p>` : '',
      statuses: inner.statuses || [],
      // V14 schema: NumberField [0=NEVER, 1=CONDITIONAL, 2=ALWAYS]
      showIcon: HIDE_ICON_CATEGORIES.has(cat)
      ...
      flags: foundry.utils.mergeObject({
        vagabond: {
          ...
          category: cat,
          ...(def.durationHint ? { durationHint: def.durationHint } : {}),
        },
      }, inner.flags || {}, { inplace: false }),
    };
  }
   ...
   *
   * V14-native: each entry becomes a top-level ActiveEffect document
   * with a hand-picked stable _id so modules can hardcode UUID refs.
   * Idempotent — existing entries with matching _id are updated in place,
   * missing ones are created. force:true wipes the pack first.
   *
   * @param {Object} [options]
   * @param {boolean} [options.force=false]  Wipe existing entries before re-creating
   * @param {string}  [options.packKey]      Override target pack (default `vagabond.effects`).
   *                                         Useful for testing against a world-scoped pack
   *                                         before the system manifest is reloaded.
   * @returns {Promise<{created:number, updated:number, total:number}>}
   */
  static async populate({ force = false, packKey } = {}) {
    packKey = packKey || `${SYSTEM_ID}.${PACK_NAME}`;
    const pack = game.packs.get(packKey);

    if (!pack) {
      ui.notifications.error(`Compendium pack "${packKey}" not found. Is it registered in system.json with type: "ActiveEffect"?`);
      return { created: 0, updated: 0, total: 0 };
    }
    if (pack.documentName !== 'ActiveEffect') {
      ui.notifications.error(`Pack "${packKey}" is type "${pack.documentName}", expected "ActiveEffect". Update system.json.`);
      return { created: 0, updated: 0, total: 0 };
    }
    ...
    try {
      ...
      if (force) {
        ...
        if (existing.length > 0) {
          ...
          await ActiveEffect.deleteDocuments(ids, { pack: packKey });
          console.log(`EffectsCompendium | force: deleted ${ids.length} existing entries`);
        }
        const existingFolders = [...pack.folders];
        if (existingFolders.length > 0) {
          await Folder.deleteDocuments(existingFolders.map(f => f.id), { pack: packKey });
          console.log(`EffectsCompendium | force: deleted ${existingFolders.length} existing folders`);
        }
      }
      ...
      // Find existing folders by category flag (idempotent)
      ...
      const foldersToCreate = SORT_ORDER
        .filter(cat => usedCategories.has(cat) && !categoryToFolderId[cat])
        .map((cat, idx) => ({
      ...
      if (foldersToCreate.length > 0) {
        const created = await Folder.createDocuments(foldersToCreate, { pack: packKey });
        for (const folder of created) {
          const cat = folder.flags?.vagabond?.category;
          if (cat) categoryToFolderId[cat] = folder.id;
        }
        console.log(`EffectsCompendium | Created ${created.length} folders`);
      }

      // ── Build AE document data, dedup, and validate ───────────────────
      const targetDatas = [];
      const seenIds = new Map();
      for (const def of EFFECT_DEFINITIONS) {
        const cat = def.category || 'misc';
        const data = this._toV14EffectData(def, categoryToFolderId[cat]);
        if (!data) {
          console.warn(`EffectsCompendium | "${def.name}" produced no data (no inner effect?), skipping.`);
          continue;
        }
        if (seenIds.has(data._id)) {
          console.warn(`EffectsCompendium | _id collision: "${def.name}" and "${seenIds.get(data._id)}" both hash to "${data._id}". Rename one or hand-pick an _id.`);
          continue;
        }
        seenIds.set(data._id, def.name);
        targetDatas.push(data);
      }

      // ── Idempotent upsert: split into create vs update by _id presence ─
      const existingByIdMap = new Map();
      for (const doc of await pack.getDocuments()) existingByIdMap.set(doc.id, doc);

      const toCreate = [];
      const toUpdate = [];
      for (const data of targetDatas) {
        if (existingByIdMap.has(data._id)) toUpdate.push(data);
      ...
      if (toCreate.length > 0) {
        ...
        for (let i = 0; i < toCreate.length; i += BATCH) {
          ...
          try {
            const c = await ActiveEffect.createDocuments(batch, { pack: packKey, keepId: true });
            createdCount += c.length;
          } catch (err) {
            console.error(`EffectsCompendium | Create batch ${Math.floor(i / BATCH) + 1} failed:`, err);
            for (const data of batch) {
              try {
                const c = await ActiveEffect.createDocuments([data], { pack: packKey, keepId: true });
                createdCount += c.length;
              } catch (e) {
                console.error(`EffectsCompendium | Failed to create "${data.name}":`, e.message);
              }
            }
          }
        }
      ...
      if (toUpdate.length > 0) {
        try {
          const u = await ActiveEffect.updateDocuments(toUpdate, { pack: packKey });
          updatedCount += u.length;
        } catch (err) {
          console.error(`EffectsCompendium | Update batch failed, falling back to per-doc:`, err);
          for (const data of toUpdate) {
            try {
              await ActiveEffect.updateDocuments([data], { pack: packKey });
              updatedCount++;
            } catch (e) {
              console.error(`EffectsCompendium | Failed to update "${data.name}":`, e.message);
            }
          }
        }
      }
      ...
      const folderSummary = Object.entries(categoryToFolderId)
        .map(([cat]) => {
          const count = targetDatas.filter(d => d.flags?.vagabond?.category === cat).length;
          return `${this.FOLDER_LABELS[cat]}: ${count}`;
        }).join(', ');

      const total = createdCount + updatedCount;
      console.log(`EffectsCompendium | created=${createdCount} updated=${updatedCount} (target ${targetDatas.length})`);
      console.log(`EffectsCompendium | ${folderSummary}`);
      ui.notifications.info(`Effects compendium: ${createdCount} created, ${updatedCount} updated.`);
