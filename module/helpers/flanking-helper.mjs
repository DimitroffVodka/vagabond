...
function _distanceFt(tokenA, tokenB) {
  ...
  // Snap to nearest grid square to handle V14 sub-pixel WASD positions
...
export const FlankingHelper = {
  ...
  _makeEffectData() {
    return {
      ...
      // Mark as temporary so the icon shows on tokens with "If Temporary" display
      duration: { rounds: 99, startRound: 0 },
      changes: [
  ...
  async _applyFlanked(actor) {
    ...
    // Force token refresh to update status icon overlay
  ...
  async _removeFlanked(actor) {
    ...
    // Remove ALL flanking effects we created (in case of duplicates)
